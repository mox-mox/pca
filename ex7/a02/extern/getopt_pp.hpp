...
struct Token
{
    ...
    Token(const std::string& value_, Type type_ = UnknownYet)
        : type(type_), value(value_), next(NULL)
    {}
...
public:
    ...
    _OptionTBase(char short_opt_, const std::string& long_opt_, T& target_)
        : short_opt(short_opt_), long_opt(long_opt_), target(target_)
    {}
    ...
    virtual Result operator()(ShortOptions& short_ops, LongOptions& long_ops, Token* /*first*/, std::ios::fmtflags flags) const
    {
        ...
        {
            LongOptions::iterator it_ = long_ops.find(long_opt);
            if (it_ != long_ops.end())
            {
                it_->second.flags = OptionData::CmdLine_Extracted;
                ret = _assign(it_->second.token, flags, short_ops);
            }
        }
...
public:
    ...
    _OptionT(char short_opt_, const std::string& long_opt_, T& target_)
        : _OptionTBase<T>(short_opt_, long_opt_, target_)
    {}
...
public:
    ...
    _DefValOption(char short_opt_, const std::string& long_opt_, T& target_, const T& default_value_)
        : BaseOption(short_opt_, long_opt_, target_), default_value(default_value_)
    {}
...
public:
    ...
    _GlobalOption(std::vector<T>& target_)
        : target(target_)
    {}
};
...
Option(char short_opt_, const std::string& long_opt_, std::string& target_, const char* def)
{
    return _DefValOption<std::string, _OptionT<std::string> >(short_opt_, long_opt_, target_, def);
}

inline _OptionT<std::string> Option(char short_opt_, std::string& target_, const char* def)
{
    return _DefValOption<std::string, _OptionT<std::string> >(short_opt_, std::string(), target_, def);
}

//  no short opt.
inline _DefValOption<std::string, _OptionT<std::string> >
Option(const std::string& long_opt, std::string& target_, const char* def)
{
    return _DefValOption<std::string, _OptionT<std::string> >(_Option::NO_SHORT_OPT, long_opt, target_, def);
}
...
public:
    ...
    // WITH long_opt:
    OptionPresent(char short_opt_, const std::string& long_opt_, bool& present_)
        : short_opt(short_opt_), long_opt(long_opt_), present(&present_)
    {}

    OptionPresent(char short_opt_, const std::string& long_opt_)
        : short_opt(short_opt_), long_opt(long_opt_), present(NULL)
    {}

    // WITHOUT long_opt:
    OptionPresent(char short_opt_, bool& present_)
        : short_opt(short_opt_), present(&present_)
    {}

    OptionPresent(char short_opt_)
        : short_opt(short_opt_), present(NULL)
    {}

    // WITHOUT short_opt
    OptionPresent(const std::string& long_opt_, bool& present_)
        : short_opt(_Option::NO_SHORT_OPT), long_opt(long_opt_), present(&present_)
    {}

    OptionPresent(const std::string& long_opt_)
        : short_opt(_Option::NO_SHORT_OPT), long_opt(long_opt_), present(NULL)
    {}
protected:
    virtual Result operator()(ShortOptions& short_ops, LongOptions& long_ops, Token* /*first*/, std::ios::fmtflags /*flags*/) const
    {
